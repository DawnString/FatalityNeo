...
public class GuideLoader extends SimplePreparableReloadListener<Void>
{
    ...
    {
        currentLang = (lang != null && !lang.isEmpty()) ? lang : DEFAULT_LANG;
    }
    ...
    public static void load(ResourceManager manager)
    {
        ...
        // 按当前语言加载，找不到则回退到 en_us
        String[] tryLangs = {currentLang};
        if (!currentLang.equals(DEFAULT_LANG))
            tryLangs = new String[]{currentLang, DEFAULT_LANG};

        for (String lang : tryLangs)
        {
            // 加载分类
            String catPath = "guide/" + lang + "/categories.json";
            var catOpt = manager.getResource(ResourceLocation.fromNamespaceAndPath(Fatality.MODID, catPath));
            ...
            {
                Fatality.LOGGER.error("Failed to load guide categories for lang {}", lang, e);
                break;
            }
        }
        ...
        {
            // 加载条目
            String entriesPrefix = "guide/" + lang + "/entries";
            var entryResources = manager.listResources(entriesPrefix, path -> path.getPath().endsWith(".json"));
            if (entryResources.isEmpty()) continue;
            ...
            for (var entry : entryResources.entrySet())
            {
                ...
                try (var reader = res.openAsReader())
                {
                    ...
                    var idStr = pathStr.substring(entriesPrefix.length() + 1, pathStr.length() - 5);
        ...
        Fatality.LOGGER.info("Loaded {} categories, {} entries (lang: {})", catMap.size(), entryMap.size(), currentLang);
    }
